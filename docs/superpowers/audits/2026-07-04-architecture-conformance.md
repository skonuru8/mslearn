...
|---|---|---|---|---|
| 1a | Adapters (§2) — 4 in spec, **5 shipped** | `mslearn/adapters/` | on-track (extended) | PDF via PyMuPDF, page locators: `adapters/pdf.py:8,23`. EPUB via ebooklib, href locators: `adapters/epub.py:21,37`. Blog via trafilatura, url+para locators: `adapters/blog.py:19-27`. YouTube captions via youtube-transcript-api, time locators: `adapters/youtube.py:43-74`. Audio via faster-whisper: `adapters/audio.py:7-18`, `transcribe.py:17-34`. **Images (plan 15, post-spec)** via a multimodal model (`image` role): `adapters/image.py`, routed in `registry.py`; transcribes text (incl. nested screenshots) + describes visuals, `image` locators; image claims carry an `image_observed` trust tier (honest provenance — model-read, not verbatim). |
