...
| 1b | Whisper fallback path (§2: caption-less yt-dlp→Whisper; audio faster-whisper) | `adapters/youtube.py`, `adapters/registry.py`, `worker/tasks.py` | **deviated (broken wiring)** | The fallback exists in code (`adapters/youtube.py:76-93` transcribes when captions fail; yt-dlp downloader at `adapters/youtube.py:29-41`) but the production ingest path never passes a transcriber: `worker/tasks.py:105` calls `load_source(ref, source_type=source_type, role=role)` with no `transcriber` kwarg. Caption-less YouTube therefore raises `TranscriptUnavailable` (`adapters/youtube.py:76-78`) → source fails. Worse: the API accepts audio uploads (`server/routers/corpus.py:84` allows `.mp3/.m4a/.wav/.flac/.ogg`) but `registry.py:48` does `kwargs["transcriber"]` → `KeyError` in the worker → every audio source fails. A transcriber can't ride through Celery task args (not serializable); it must be constructed in the worker. Never live-tested, as suspected. |
...
| 6 | Neo4j property graph + vector indexes + export portability rule (§3) | `mslearn/graph/store.py`, `graph/export.py`, `pipeline/exports.py` | on-track (one caveat) | Labels/rels as specced: `EXTRACTED_FROM` `store.py:205`, `IN_CONCEPT` `store.py:358`, `DEPENDS_ON` `store.py:371`, `CONFLICTS_WITH {classification, rationale}` `store.py:386-392`. Native vector indexes on claims+chunks: `store.py:39-42,95,288-334`. GraphML+JSON dump: `graph/export.py:8-16`, `pipeline/exports.py:80-88`. Caveat: "every export run also dumps the graph" is honored by the default request body and by the UI (which always sends all three kinds, `frontend/src/components/AdminBar.tsx:82`), but the API permits `{"kinds": ["markdown"]}` with no graph dump (`server/routers/exports.py:19,28-34`) — a caller can opt out of the portability rule. |
| 7 | mem0 learner-memory integrity rule (§3b) | `mslearn/memory/mem0_impl.py`, `server/routers/chat.py`, `pipeline/quiz.py`, `server/routers/memory.py` | partial | mem0 on the same Neo4j: `memory/mem0_impl.py:38-45`. Personalization-only enforced by prompt framing ("PERSONALIZATION ONLY" prefix on every hint, `server/routers/chat.py:138`) and by construction — facts are only retrieved from the graph (`chat.py:83` retrieval vs memory hints are separate prompt sections). Quiz failures recorded to memory: `pipeline/quiz.py:138,197-202`. Memory panel inspect/delete: `server/routers/memory.py:12,21`, `frontend/src/views/MemoryView.tsx`. Provenance eval exists and is real when run non-offline: `evals/judged.py:119-134`, wired into the runner (`evals/runner.py:32`) and into self-evolution gating (`evals/evolve.py:126`). Gaps: offline runs default `provenance.violations` to 0.0 (`runner.py:34`) so the gate is vacuous without live backends, and it has never been run live. Minor: mem0 embedder model `"nomic-embed-text"` is hardcoded (`mem0_impl.py:49`), violating "model IDs live in config, never code". |
| 8 | Celery queues: 3 (ingest/judge/transcribe, whisper exclusive) (§1) | `worker/app.py`, `Makefile`, `scripts/dev_up.sh` | **deviated (needs decision)** | 2 queues, not 3: routes at `worker/app.py:20-24` (chunk/extract→ingest, synthesize→judge); workers `-Q ingest --concurrency=2` and `-Q judge --concurrency=1` (`Makefile:47-51`, `scripts/dev_up.sh:70-74`). No `transcribe` queue; transcription (once wired, see row 1b) would run inside `chunk_source_task` on ingest. Memory-ceiling implication on the 18 GB M3: spec made whisper exclusive with heavy ingest precisely because 2 concurrent ingest slots + a whisper model + Ollama can collide; with concurrency=2, two simultaneous transcriptions are possible. Judge concurrency is 1 vs spec's ~8 — deliberate post-spec choice (plan 12: synthesis must not starve ingest; a single serial synthesis run is the dedup model). Accept or fix alongside the row-1b transcriber wiring. |
...
- [ ] Wire a whisper transcriber into the worker ingest path (`chunk_source_task` →
      `load_source`) so audio uploads and caption-less YouTube stop failing —
      the UI accepts both today and both break in the worker (row 1b).
- [ ] Decide the transcription queue question (row 8): either add the spec's
      `transcribe` queue (concurrency 1, exclusive with heavy ingest) or
      explicitly accept whisper-on-ingest with a documented memory budget for
      the 18 GB machine.
...
- [ ] Build the frontend eval report/review page (spec §6 "UI report page";
      the API endpoints exist, nothing in the UI calls them).
- [ ] Move the mem0 embedder model id (`nomic-embed-text`, `mem0_impl.py:49`)
      into `profiles.yaml`/settings to restore "model IDs live in config, never code".
- [ ] Close the export-portability loophole: either make every export request
      include the graph dump server-side or document the opt-out as accepted.
