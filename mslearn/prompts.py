...
PROMPTS: dict[str, str] = {
    ...
    "guide": (
        ...
        " -- what it means and why it matters -- never copy the source wording or a"
        " near-paraphrase (same structure with swapped synonyms). Set 'text' to your"
        " rewrite, 'kind' to the claim's kind, and 'claims' to the id(s) of the"
        " claim(s) it rests on.\n"
        "- Every supplied claim id must be covered by exactly one grounded item."
        " Never invent items, text, or claim ids.\n"
        "- Group items into 2-6 sections; 'skeleton' lists section titles in order."
        " Give each section a short id (s1,s2,...).\n"
        "- 'tl_dr.text' is one plain sentence citing the 1-2 claim ids it rests on"
        " in tl_dr.claims.\n"
        "- 'open_questions' may list gaps the source did NOT cover, or be empty."
        " It is the only field not tied to a claim; never put facts there.\n"
        "- Also produce an 'interpretation' array -- YOUR analysis of these claims,"
        " shown to the reader as your analysis (not source fact). Choose only the"
        " angles that yield real content from: assumption (what must be true for"
        " these to hold), evidence (how strong/what kind), steelman (strongest"
        " objection + whether it holds), verdict (do you buy it, and why / what's"
        " missing), synthesis (what this adds up to). Set 'angle' to one of those"
        " exact words, 'text' to your reasoning; you MAY cite claim ids in 'claims'"
        " but need not. Omit an angle, or the whole array, if the concept is too"
        " thin for real analysis -- never pad.\n"
